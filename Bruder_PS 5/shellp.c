...
{
  int stageRem = c->_nbs; //remaining stages - while not 0 we will keep executing stages
  int stageCur = 0; //current stage
  
  //check mode of c
  //if mode is R_NONE do nothin
  //if mode is input do first stages input to the fd of fopen(input, r)
  //if mode is output do last stages output to the fd of fopen(output, w)
  
  if(c->_mode == R_INP){
      FILE* fp = fopen(c->_input, "r");
      int fd = fileno(fp);
      c->_stages[0] = setStageInput(c->_stages[0], fd);
  }
  
  //while loop using nbs from c which specifies number of stages
  while(stageRem != 0){
      //create pipes for stages to communicate
      int s1TOs2[2];
      pipe(s1TOs2);
      
      if(c->_mode == R_OUTP && stageRem == 1){
          FILE* fp = fopen(c->_output, "w");
          int fd = fileno(fp);
          c->_stages[stageCur] = setStageOutput(c->_stages[stageCur], fd);
      }
      else if(c->_mode == R_APPD && stageRem == 1){
        FILE* fp = fopen(c->_output, "a");
        int fd = fileno(fp);
        c->_stages[stageCur] = setStageOutput(c->_stages[stageCur], fd);
      }
      else if(stageRem == 1){
        ...
        //use setstageinput and setstageoutput with fd's from pipes for inter-stage comms
        c->_stages[stageCur] = setStageOutput(c->_stages[stageCur], s1TOs2[1]);
        c->_stages[stageCur+1] = setStageInput(c->_stages[stageCur+1], s1TOs2[0]);
      }
      
      //spawnstage executes a stage
      spawnStage(c->_stages[stageCur]);
      waitStage(c->_stages[stageCur]);
...
int basicExecute(char* com,int mode,char* input,char* output,char** args)
{
  ...
  if(pid1 == 0){
    ...
    if(mode == R_NONE){ //no redirections
        execvp(com, args);
    }
    else if(mode == R_INP){ //input redirected
    ...
    else if(mode == R_OUTP){ //output redirected
    ...
    else if(mode == R_APPD){ //output redirected and append to eof
